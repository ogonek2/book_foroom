...
@section('profile-content')
    <div id="app">
            ...
            <div class="flex space-x-1 bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
                @if (auth()->check() && auth()->user()->id === $user->id)
                    <a href="{{ route('libraries.create') }}"
                        class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-sm rounded-lg font-medium hover:from-purple-600 hover:to-pink-600 transition-all">
                        <i class="fas fa-plus mr-2"></i>
                       Нова добірка
                    </a>
                @endif
                <button @click="activeTab = 'my'"
                    :class="activeTab === 'my' ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-white shadow-sm' :
                        'text-gray-600 dark:text-gray-400'"
                    class="px-4 py-2 rounded-md text-sm font-medium transition-all duration-200">
                    Мої добірки
                </button>
                <button @click="activeTab = 'saved'"
                    :class="activeTab === 'saved' ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-white shadow-sm' :
                        'text-gray-600 dark:text-gray-400'"
                    class="px-4 py-2 rounded-md text-sm font-medium transition-all duration-200">
                    Збережені добірки
                </button>
        ...
        <div v-show="activeTab === 'my'">
                            ...
                            <div class="flex items-start justify-between mb-4">
                                ...
                                @if (auth()->check() && auth()->user()->id === $user->id)
                                    <div class="relative">
                                        ...
                                        <!-- Dropdown Menu -->
                                        <div v-if="activeMenuId === {{ $library->id }}" @click.stop
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    @for ($i = 0; $i < 3; $i++)
                                        ...
                                        @elseif($i == 2 && $library->books_count > 3)
                                            <div
                                                class="w-12 h-16 bg-gradient-to-br from-orange-400 to-red-500 rounded-lg flex items-center justify-center backdrop-blur-sm">
                                                <span
                                                    class="text-white font-bold text-xs">+{{ $library->books_count - 2 }}</span>
                                            </div>
                                        @else
                                            <div
                                                class="w-12 h-16 bg-gray-300 dark:bg-gray-600 rounded-lg flex items-center justify-center">
            ...
            @else
                ...
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mb-6">Створіть першу добірку для збереження улюблених книг
                    </p>
                    @if (auth()->check() && auth()->user()->id === $user->id)
                        <a href="{{ route('libraries.create') }}"
        ...
        <div v-show="activeTab === 'saved'">
            @if (isset($savedLibraries) && $savedLibraries->count() > 0)
                <!-- Saved Libraries Grid -->
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                    @foreach ($savedLibraries as $library)
                        <div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-slate-700/30 p-6 hover:shadow-2xl transition-all duration-300 cursor-pointer group"
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center space-x-4 text-sm text-slate-500 dark:text-slate-400">
                                        ...
                                        <span class="flex items-center space-x-1">
                                            ...
                                            @else
                                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                                        d="M10 18a8 8 0 100-16 8 8 0 000 16zM4.332 8.027a6.012 6.012 0 011.912-2.706C6.512 5.73 6.974 6 7.5 6A1.5 1.5 0 019 7.5V8a2 2 0 004 0 2 2 0 011.523-1.943A5.977 5.977 0 0116 10c0 .34-.028.675-.083 1H15a2 2 0 00-2 2v2.197A5.973 5.973 0 0110 13v-2a2 2 0 00-2-2 2 2 0 00-1.668-1.973z"
                                                        clip-rule="evenodd" />
                                                </svg>
                                                <span>Публічна</span>
                                            @endif
                                    ...
                                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
                                        Автор: {{ $library->user->first_name ?? ($library->user->name ?? 'Невідомий') }}
                                    </p>
                                </div>
                                ...
                                <div class="relative">
                                    ...
                                    <!-- Dropdown Menu -->
                                    <div v-show="activeMenuId === {{ $library->id }}"
                                        class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-700 rounded-md shadow-lg z-10 border border-slate-200 dark:border-slate-600">
                                        <div class="py-1">
                                            <button @click.stop="unsaveLibrary({{ $library->id }})"
                                                class="flex items-center w-full px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-600">
                            ...
                            <div class="flex space-x-2 mb-4">
                                @php
                                    $previewBooks = $library->books->take(3);
                                @endphp
                                @for ($i = 0; $i < 3; $i++)
                                    @if ($i < $previewBooks->count())
                                        <div class="flex-shrink-0">
            ...
            @else
                <!-- Empty State for Saved Libraries -->
                <div class="text-center py-12">
                    <div class="text-slate-400 dark:text-slate-500 text-6xl mb-4">
                        <svg class="w-24 h-24 mx-auto" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"
                                clip-rule="evenodd" />
                        </svg>
                    </div>
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Збережених добірок немає</h3>
                    <p class="text-gray-600 dark:text-gray-400 mb-6">Знайдіть цікаві добірки та збережіть їх для
                        майбутнього перегляду</p>
                            ...
                            if (confirm(
                                ...
                                try {
                                    ...
                                    if (data.success) {
                                        ...
                                        this.showNotification(data.message || 'Помилка при видаленні добірки',
                                            'error');
                                    }
                                } catch (error) {
                        ...
                        async unsaveLibrary(libraryId) {
                            if (confirm('Ви впевнені, що хочете прибрати цю добірку з збережених?')) {
                                try {
                                    ...
                                    if (data.success) {
                                        this.showNotification('Добірку прибрано з збережених!', 'success');
                                        setTimeout(() => {
                                            location.reload();
                                        }, 1000);
                                    } else {
                                        this.showNotification(data.message ||
                                            'Помилка при видаленні з збережених', 'error');
                                    }
                                } catch (error) {
                                    console.error('Error unsaving library:', error);
                                    this.showNotification('Помилка при видаленні з збережених', 'error');
                                }
                            }
                            this.activeMenuId = null;
                        },
